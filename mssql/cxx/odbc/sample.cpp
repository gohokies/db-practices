...
int main()
{
    ...
    SQLCHAR strConnect[] = "Driver=ODBC Driver 17 for SQL Server;Server=localhost;Uid=sa;Pwd=SQLPassword!0;database=master";
    int i = 0;
    ...
    retcode = SQLDriverConnect(hdbc, NULL, strConnect, SQL_NTS,
        outstr, sizeof(outstr), &outstrlen, SQL_DRIVER_NOPROMPT);
    
    CHECK_ERROR(retcode, "SQLDriverConnect", hdbc, SQL_HANDLE_DBC);

    // Allocate statement handle
    retcode = SQLAllocHandle(SQL_HANDLE_STMT, hdbc, &hstmt);
    CHECK_ERROR(retcode, "SQLAllocHandle(SQL_HANDLE_STMT)", hstmt, SQL_HANDLE_STMT);

    retcode = SQLExecDirect(hstmt, (SQLCHAR*) "SELECT @@Version", SQL_NTS);
